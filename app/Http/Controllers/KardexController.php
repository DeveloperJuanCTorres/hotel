...
class KardexController extends Controller
{
    ...
    private function obtenerMovimientos($request)
    {
        $productId = $request->product_id;
        $desde = $request->from_date;
        $hasta = $request->to_date;
        ...
        $movs = DB::query()
            ->fromSub($entradas->unionAll($salidas), 'k')
            ->orderBy('fecha')
            ->get();
        ...
        $movimientos = $movs->map(function ($m) use (&$stock, &$costoTotal, &$costoProm) {
            ...
            if ($m->tipo == 'COMPRA') {
                ...
                $costoTotal += $m->cantidad * $m->precio_unitario;
                $stock += $m->cantidad;
                $costoProm = $stock > 0 ? $costoTotal / $stock : 0;
            } else {

                $salida = abs($m->cantidad);
                $costoTotal -= $salida * $costoProm;
                $stock -= $salida;
            }
        ...
        $stockFinal = DB::table('products')->where('id', $productId)->value('stock');

        return compact('movimientos', 'stockFinal');
    }

    public function exportExcel(Request $request)
    {
        $data = $this->obtenerMovimientos($request);

        return Excel::download(
            new KardexExport($data['movimientos']),
            'Kardex.xlsx'
        );
    }
