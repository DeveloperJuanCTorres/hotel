...
class MovementController extends Controller
{
    ...
    public function generar(Request $request)
    {
        try {
            ...
            if ($request->tipo === 'ticket') {
                return $this->generarTicketInterno($request);
            }

            return $this->generarComprobanteSunat($request);
     ...
     * ===============================
     */
    private function generarTicketInterno(Request $request)
    {
        $invoice = Invoice::where('name', 'Ticket')->first();
        ...
        if (!$invoice) {
            return response()->json([
                'status' => false,
                'msg' => 'No existe configuración para Ticket'
            ]);
        }
        ...
        $movimiento->referencia = $serie . '-' . $numero;
        $movimiento->total = $request->total;
        $movimiento->estado_sunat = null;
        $movimiento->observacion_sunat = 'Ticket interno';
        $movimiento->save();
        ...
        Room::where('numero', $request->habitacion)
            ->update(['status' => 'LIMPIEZA']);

        // Generar PDF del ticket
        $pdf = Pdf::loadView('rooms.partials.pdf_ticket', [
            'serie'       => $serie,
            'numero'      => $numero,
            'cliente'     => $request->name,
            'habitacion'  => $request->habitacion,
            'total'       => $request->total,
            'dias'        => $request->total_dias,
            'fecha'       => now()->format('d/m/Y H:i'),
        ])->setPaper([0, 0, 226.77, 600]); // 80mm POS
        ...
        $ruta = "tickets/ticket_{$serie}_{$numero}.pdf";
        $pdf->save($dir . "/ticket_{$serie}_{$numero}.pdf");
    ...
    /**
     * ===============================
     * BOLETA / FACTURA (NUBEFACT)
     * ===============================
     */
    private function generarComprobanteSunat(Request $request)
    {
        $business = Business::first();
        ...
        switch ($tipo) {
            ...
            default:
                return response()->json(['status' => false, 'msg' => 'Tipo no válido']);
        }

        if (!$invoice) {
            return response()->json([
                'status' => false,
                'msg' => 'No existe configuración para ' . $tipo
            ]);
        }
        ...
        switch ($request->tipo_doc) {
            ...
            case 'RUC': $tipo_doc = 6; break;
            case 'PASAPORTE': $tipo_doc = 7; break;
            default:
                return response()->json(['status' => false, 'msg' => 'Tipo de documento no válido']);
        }
        ...
        $product = [
            ...
            "codigo" => "Z001",
            "codigo_producto_sunat" => "10000000",
            "descripcion" => "Servicio de alojamiento Hab. " . $request->habitacion,
            "cantidad" => $request->total_dias,
            "valor_unitario" => $valor_unitario_sin_igv,
            "precio_unitario" => $request->valor_unitario,
            "subtotal" => $valor_unitario_sin_igv * $request->total_dias,
        ...
        $movimiento->total = $request->total;
        $movimiento->estado_sunat = $resp->aceptada_por_sunat;
        $movimiento->observacion_sunat = $resp->sunat_description ?? null;
        $movimiento->pdf_sunat = $resp->enlace_del_pdf ?? null;
        $movimiento->save();
